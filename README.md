...
- Изменения сразу отображаются в UI и сохраняются в `localStorage`.
- После перезагрузки локальные изменения поднимаются из `localStorage`.
- Если локального состояния нет, приложение берет стартовые данные из API.
