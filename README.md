...
- `GET /orders` используется для первичной загрузки данных из MockAPI.
- `Создать`, `Сменить статус`, `Удалить` выполняются локально через Redux actions.
- Все изменения сохраняются в `localStorage`, поэтому не пропадают после перезагрузки.
- Если очистить `localStorage`, приложение снова возьмет стартовые данные из API.
...
- Для тестов использован Vitest, так как он хорошо подходит для Vite, настраивается проще и по синтаксису почти как Jest.
